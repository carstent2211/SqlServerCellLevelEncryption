...
namespace EF_SqlServerEncryption {
    public static class ObjectContextOperations<T> where T : ObjectContext, new() {
        private static ObjectContext _objectContext = null;
        private static DbContext _dBContext = null;

        static ObjectContextOperations() {
            _objectContext = new T();
            _dBContext = new DbContext(_objectContext, true);
        }
